...
async function fetchData() {
  ...
  const [cs, bu] = await Promise.all([
    supabase
      .from('customers')
      .select(`
        id, phone, name, gender, email, dob, customer_type,
        primary_business_unit_id, status,
  ...
  return { customers: cs.data ?? [], businessUnits: bu.data ?? [] };
}
...
export default async function CustomersPage() {
  const { customers, businessUnits } = await fetchData();
              ...
              customers.map((c) => {
                ...
                return (
                  <TableRow key={c.id}>
                    ...
                    <TableCell>
                      <CustomerRowActions customer={{ ...record, status: c.status }} businessUnits={businessUnits} />
                    </TableCell>
                  </TableRow>
                );
              })
